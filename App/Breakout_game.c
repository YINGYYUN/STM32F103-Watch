...
static void Render(void)
{
	...
	else
	{
		...
		{
			OLED_ShowString( 44, 28, "PAUSE", OLED_8X16);
		}
	}
...
int BreakoutGame(void)
{
	...
	{
		/* 确认键：发射/暂停/确认退出 */
		uint8_t confirm = Key_Check(KEY_NAME_COMFIRM, KEY_SINGLE);
		
		if (state == STATE_WAIT)
		{
			// 发射
			if (confirm) { LaunchBall(); state = STATE_PLAYING; }
		}
		else if (state == STATE_PLAYING)
		{
			// 暂停
			if (confirm) state = STATE_PAUSED;
		}
		else if (state == STATE_PAUSED)
		{
			// 取消暂停
			if (confirm) state = STATE_PLAYING;
		}
